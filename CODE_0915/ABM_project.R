...
catastrophe = c( 0, 1.1, 1.2, 1.3)   ## 是否发生巨灾风险  0：不发生  1：发生
...
risk_aversion_par = 0.7 ## 风险厌恶系数 标准情况下冲击系数为1  我们将它扩大一定的倍数
...
data_raw <- filter(data_raw, loss != 0)
data_raw$load = data_raw$fee_1+data_raw$fee_2+data_raw$fee_3

uni_com <- unique(data_raw$company)
data <- tibble(company = uni_com) 
data$asset =  filter(data_raw,year == 2021)[,3]
...
x0=c(0.2, 0.2, 0.6)
rm_top = 0.3 ## 对于股票的上限
rm_rate_mean_new = 0.08#0.077353545
rm_rate_sd_new = 0.2#0.12668
rf_rate_mean = 0.05#0.0238
rf_rate_sd = 0.05#0.00248
cash_rate_mean =  0.02
cash_rate_sd = 0.001#0

## loss ratio
data_raw$loss_ratio = data_raw$loss/c(data_raw$premium_revenue - data_raw$load)

# leverage
data$equity = filter(data_raw,year == 2021)[,5]
data$pre_earned = filter(data_raw,year == 2021)[,7]#333333333333333
data$load = filter(data_raw,year == 2021)[,13]
LR_basic = rep(0.99, n_max)#data$loss_ratio##################################这里修改了基础赔付率水平
beta0 <- data$beta0

###
p_basic <- 1000 
beta0 <- - 0.0332*(log(data$pre_earned)) + 0.576

## 保单的增长率
g <-  rep(1.05, t_final)#rep(1.06, t_final) #exp(c(t_final:1)/10/t_final)
K0 <- c(data$asset) * 1000000
leverage = data$equity/data$asset
E0 <-  703043520 * 1 #sum(data$pre_earned - data$load) * 1000000 / p_basic   ## 最初的市场总风险单位数  

################################################
## 参数部分
k1 <- 0.667 #  # 保费对纯保费赔付率的敏感性参数
k3 <- 0.25  # 公司上一期提取的附加保费对这一期的影响
k4 <- 0.5 #0.5  # 分红比率  
k5 <-rep(0.05, t_final) #  保险市场吸引资本进入的利润率阈值#######
k6 <- 30 # 被视作每单位超过阈值水平的利润率对新资本的吸引能力 
k9 <- 0.001 #0.01   # 价格弹性
k10 <-  5   ## 风险资产的价格下降速度

## 固定值部分
LR_sum0 = 1
LR_sum_basic = mean(data$LR_basic)
n_new_max <- 2 ## 新进入保险 公司数量的最大值


## 资产冲击规模按照风险资产规模比例分配
#time <- 10 ## 循环期数
#k14 <- 1.05 ## 无风险资产 ， 公司内部的流动性要求


normal <- 1 
for(loop_b in 1:8000){
  
  ## 生成每家公司的每年度的投资收益率
  
  rm_rate = matrix(0, nrow = t_final + 10 , ncol = n_max)
  for(i in 1 : n_max){
    rm_rate[, i] =  rnorm(t_final + 10 , rm_rate_mean_new, rm_rate_sd_new)
  }
  
  rf_rate <- rnorm(100, rf_rate_mean, rf_rate_sd) #rep(0.02, t_final)#* 0.05 *exp(-c(1:t_final)/20)
  ## 用来确定无风险投资比例的函数
  ## 使用的是期望收益率-期望方差
    ...
    if(normal <= time){
      ...
      repeat {
        ...
        result <- tryCatch({
          ...
          rho <- c()
          rho = data$pre_earned/sum(data$pre_earned)
          ...
          alpha_zhai <- matrix(nrow = t_final, ncol = n_max)  ## 投资在债券的比例
          alpha_gu <- matrix(nrow = t_final, ncol = n_max)  ## 投资在股票的比例
          alpha_cash <- matrix(nrow = t_final, ncol = n_max)  ## 投资在存款的比例
          ...
          FV_free_before <- matrix(0, nrow = t_final, ncol = n_max) ## 减价出售过程中风险资产
          FV_risk_before <- matrix(0, nrow = t_final, ncol = n_max) ## 减价出售过程中风险资产
          ...
          for (i in c(1:length(rho))) { ## length(rho)为t期公司数量
            ...
            fr <- function(alpha,
                           rf_mean = rf_rate_mean,
                           rf_sd = rf_rate_sd,
                           rm_mean = mean(rm_rate[(len_rate_history + t-back_year):(len_rate_history + t-1), i]), 
                           rm_sd = sd(rm_rate[(len_rate_history + t-back_year):(len_rate_history + t-1), i]),
                           cash_mean = cash_rate_mean,
                           cash_sd = cash_rate_sd,
                           risk_aversion = risk_aversion_par) {
              
              x1 <- alpha[1]
              x2 <- alpha[2]
              x3 <- alpha[3]
              return(-sum(c(x1, x2, x3) * c(rf_mean, rm_mean, cash_mean) * (1- risk_aversion)) + 
                       risk_aversion * t(as.matrix(c(x3, x1, x2))) %*% 
                       invest_matrix  %*% as.matrix(c(x3, x1, x2)))
            }
            
            confun = function(alpha){
              f = NULL
              f = rbind(f, 
                        - data$equity[i]*1000000 /mc_function(yewu = E0*rho[i]*P_p[t, i],
                                                              invest = invest[t, i],
                                                              alpha = alpha,
                                                              #lilv = 0.025,
                                                              x  = 0,
                                                              rf0_feishou = mc_rf0[i]) + sl_ratio_floor)
              f = rbind(f, -alpha[1] + 0.1)
              f = rbind(f, alpha[2] - rm_top)
              f = rbind(f, -alpha[3] + 0.1)
              pp = NULL
              pp = rbind(pp, alpha[1] + alpha[2] + alpha[3] -1)
              return(list(ceq=pp,c=f))
            }
            
            alpha_zichan  = solnl(x0, objfun = fr, confun=confun , lb =c(0,0,0), ub = c(1,1,1),
                                  tolX = 1e-02,
                                  tolFun = 1e-03, tolCon = 1e-02, maxnFun = 1e+05, maxIter = 4000)$par
            alpha_zhai[t, i] = alpha_zichan[1]
            alpha_gu[t, i] = alpha_zichan[2]
            alpha_cash[t, i] = alpha_zichan[3]
            ## 第一期新公司使用与设定值
            ## 期末无风险现金流:无风险资产比重*总资产*（1+无风险收益率）
            FV_free[t, i] = alpha_zhai[t, i] * invest[t, i] * (1 + rf_rate[t])
            cash[t, i] = alpha_cash[t, i] * invest[t ,i] * (1 + cash_rate_mean)
          ...
          for (i in 1:length(rho)) {
            if(FV_free[t, i] > 0 ){
              if(L[t, i] > (FV_free[t, i] + cash[t, i])){ ## 如果赔付支出大于公司所能提供的流动性之和
                liq_gap[t, i] = L[t, i] - FV_free[t, i] - cash[t, i] ## 流动性缺口规模
          ...
          sum_risk = sum(FV_risk_before[t, ])
          pp = 0
          p = 1
          while (round(sum_lip) > 0 ) {
            # 先计算出售的价格
            #sum_v = sum_lip/p
            p_n =  p * exp( - k10 * sum_lip/p / sum_risk)  ## 价格的下降幅度
            FV_risk[t, ] = (FV_risk[t, ] - liq_gap[t, ]/p)
          ...
          n_exit <- c()
          for(i in 1:length(rho)){
            FV_final[t, i] = FV_free[t, i] + p * FV_risk[t, i] + cash[t, i]
            # 经营结果
            pi[t, i] = FV_final[t, i]  - invest[t, i] + #投资利润
              E[t, i] * P_p[t, i] - L[t, i] ## 承保利润  ## 现阶段的利润
            ## 算是否需要进行偿付能力调整
            Equity[t, i] = leverage[i] * K0[i] + pi[t, i]   ## 经营结束之后的权益量
          ...
          beta_avg = c(beta_avg, as.numeric(quantile(beta[t,][which(E[t,] != 0)], 0.75)))
          ## 第一期经营结束==================================================
          ...
          for (i in 1:length(rho)) {
            if(FV_free[t, i] > 0 ){
              if(L[t, i] > (FV_free[t, i] + cash[t, i])){ ## 如果赔付支出大于公司所能提供的流动性之和
                ...
                ## 如果存在流动性缺口，那么就需要进行减价出售机制
                ## 如果没有缺口那就不进行减价出售
                cash[t, i] = cash[t, i] + liq_gap[t, i] ## 减价出售结束之后现金账户的价值
              }
            }
          }
          ## 减价出售机制设定
          ...
          if (n_new[t-1] != 0){
            ...
            for (i in c((length(rho)- n_new[t-1] + 1) : length(rho))){
              ...
              Equity[t-1, i] = leverage[i] * K_begin[t, i]
              E[t, i] = E_sum[t]  * rho[i]
              L[t, i] = rpois(1, E[t, i]) * rgamma(1, shape = LR_basic[i] * p_basic /2 , scale = 2) 
          ...
          for (i in 1:length(rho)) {
            if(FV_free[t, i] > 0 ){
              if(L[t, i] > (FV_free[t, i] + cash[t, i])){ ## 如果赔付支出大于公司所能提供的流动性之和
                ...
                ## 如果存在流动性缺口，那么就需要进行减价出售机制
                ## 如果没有缺口那就不进行减价出售
                #FV_free[t, i] = FV_free_before[t, i] 
                cash[t, i] = cash[t, i] + liq_gap[t, i]
                #FV_risk[t, i] = FV_risk_before[t, i] -  liq_gap[t, i]/0.8
              }
            }
          }
          ## 减价出售机制设定
          ...
          for(t in c(4 : t_final)){    ## t_final >= 4, 整体循环，回括号在最后一行
            ...
            if(is.numeric(n_exit)){     
              for (i in setdiff(setdiff(c(1:(length(rho)-n_new[t-1])), n_recover), n_exit)) {
                ...
                alpha_zichan = c(0, 0, 1)
                tryCatch( {alpha_zichan  = solnl(x0, objfun = fr, confun=confun , lb =c(0,0,0), ub = c(1,1,1),
                                                 tolX = 1e-02,
                                                 tolFun = 1e-03, tolCon = 1e-02, maxnFun = 1e+05, maxIter = 4000)$par},
          ...
          for ( ww in 1:t_final){
            ...
            for(i in 1:n_max){
              s = Equity[ww, i] / mc_function_asset(yewu = E[ww, i] * P_p[ww, i], 
                                                    FV_free = FV_free[ww, i],
                                                    FV_risk = FV_risk[ww, i],
                                                    cash = cash[ww,i],
                                                    x = 0 ,
                                                    rf0_feishou = mc_rf0[i])
              
              if (is.na(s) == F & s > 0 & s < 10){
...
PATH1 = "F://jianguo/newest/data_0420/risk_aver_0.7/"
for (shock_und in catastrophe){
